...
contract TestCallerFarmBeetsBar {
  FarmBeetsBar strat;

  constructor(address _strat) public {
    strat = FarmBeetsBar(_strat);
  }
...
contract FarmBeetsBarTest is DSTest {
  FarmBeetsBar strat;
  MockERC20 reward;
  MockBeetsBar underlying;
  MockERC20 fBeetsBPT;
  MockMasterChef chef;
  IHevm hevm;
  uint basePID;
  ...
  function setUp() public {
    ...
    fBeetsBPT = new MockERC20("bar", "b");
    underlying = new MockBeetsBar(address(fBeetsBPT));
    ...
    poolTokens[0] = address(fBeetsBPT);

    MockBalancerVault balVault = new MockBalancerVault();
    balVault.setPoolTokens(poolID[0], poolTokens);
    balVault.setGetPool(poolID[0], address(fBeetsBPT));

    basePID = 0;
    strat = new FarmBeetsBar(
    ...
    MockERC20(underlying).mint(address(this), 1000 ether);
    fBeetsBPT.mint(address(balVault), 1000 ether);
  }

  function test_permissions() public {
    TestCallerFarmBeetsBar caller = new TestCallerFarmBeetsBar(address(strat));
  ...
  function test_harvest() public {
    ...
    (uint balB4, uint pending) = chef.userInfo(basePID, address(strat));
    assertGt(pending, 0);

    strat.harvest();

    uint bal;
    (bal, pending) = chef.userInfo(basePID, address(strat));
