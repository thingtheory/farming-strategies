...
contract RewardsToFBeetsTest is DSTest {
  ...
  function setUp() public {
    ...
    balVault.setGetPool(poolID[1], address(fBeetsBPT));

    basePID = 0;
    fBeetsPID = 1;
    SimpleVault vault = new SimpleVault("foobar", "fb", 0);
    strat = new RewardsToFBeets(
      basePID, fBeetsPID, address(chef), address(this), address(balVault),
      address(reward), poolID[0], poolID[1]
    );
